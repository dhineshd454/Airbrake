...
function makeGroup(overrides: Partial<BreakGroup> = {}): BreakGroup {
  return {
    id: 'group-1',
    fingerprint: 'abc123',
    applicationId: 'app-123',
...
function makeRepos(existingGroup: BreakGroup | null = null) {
  const savedGroups: BreakGroup[] = [];
  const updatedGroups: BreakGroup[] = [];
  const savedBreaks: Array<Break & { groupId: string }> = [];
  const indexedBreaks: Array<Break & { groupId: string }> = [];
  const indexedGroups: BreakGroup[] = [];

  const breakGroups: BreakGroupRepository = {
    findByFingerprint: jest.fn().mockResolvedValue(existingGroup),
    save: jest.fn().mockImplementation(async (g: BreakGroup) => {
      savedGroups.push(g);
      return g;
    }),
    update: jest.fn().mockImplementation(async (g: BreakGroup) => {
      updatedGroups.push(g);
      return g;
    }),
  };

  const breaks: BreakRepository = {
    save: jest.fn().mockImplementation(async (b: Break & { groupId: string }) => {
      savedBreaks.push(b);
    }),
  };

  const indexer: SearchIndexer = {
    indexBreak: jest.fn().mockImplementation(async (b: Break & { groupId: string }) => {
      indexedBreaks.push(b);
    }),
    indexBreakGroup: jest.fn().mockImplementation(async (g: BreakGroup) => {
...
describe('DefaultErrorAggregator', () => {
  describe('new break (no existing group)', () => {
    it('classifies as "new" and creates a BreakGroup with status "open"', async () => {
      ...
      const aggregator = new DefaultErrorAggregator(breakGroups, breaks, indexer);
      const b = makeBreak();
    ...
    it('sets the fingerprint on the new group from the break fields', async () => {
      ...
      const aggregator = new DefaultErrorAggregator(breakGroups, breaks, indexer);
      const b = makeBreak();
      const expectedFingerprint = computeFingerprint(b);
  ...
  describe('known payload produces expected BreakGroup', () => {
    ...
    it('calls breakRepo.save with the correct groupId (PostgreSQL write verified)', async () => {
      ...
      expect(breaks.save).toHaveBeenCalledWith(
        expect.objectContaining({ groupId: result.group.id }),
      );
    });
    ...
    it('calls indexer.indexBreak and indexer.indexBreakGroup (Elasticsearch write verified)', async () => {
      ...
      expect(indexer.indexBreak).toHaveBeenCalledWith(
        expect.objectContaining({ groupId: result.group.id }),
      );
  ...
  describe('occurrenceCount increments across multiple aggregations', () => {
    ...
    it('increments occurrenceCount correctly across three sequential aggregations', async () => {
      ...
      let currentGroup: BreakGroup | null = null;
      ...
      const breaks: BreakRepository = { save: jest.fn().mockResolvedValue(undefined) };
      const indexer: SearchIndexer = {
