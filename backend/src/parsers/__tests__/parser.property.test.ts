...
const validEnvironments = ['production', 'qa', 'development'] as const;
...
const arbitraryValidLogPayload = () =>
  fc.record({
    id: fc.uuid(),
...
describe('Property 22: Parsing Produces Valid Normalized Records', () => {
  ...
  it('for any valid log payload, parseLogRecord returns success with all required fields populated', async () => {
    await fc.assert(
      fc.asyncProperty(arbitraryValidLogPayload(), async (payload) => {
        const result = await parseLogRecord(payload);
...
const arbitraryMalformedPayload = () =>
  fc.oneof(
...
describe('Property 23: Malformed Payload Handled Without Crash', () => {
  ...
  it('for any malformed payload, parseLogRecord never throws and always returns { success: false }', async () => {
    await fc.assert(
      fc.asyncProperty(arbitraryMalformedPayload(), async (payload) => {
        let result: Awaited<ReturnType<typeof parseLogRecord>>;

        // Must never throw
        try {
          result = await parseLogRecord(payload);
        } catch (err) {
...
import { serializeLogRecord } from '../logParser';
...
describe('Property 24: Parse-Serialize-Parse Round-Trip', () => {
  ...
  it('parse → serialize → parse produces a record equal to the first parse result', async () => {
    await fc.assert(
      fc.asyncProperty(arbitraryValidLogPayload(), async (payload) => {
        // First parse
        const result1 = await parseLogRecord(payload);
