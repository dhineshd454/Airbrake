import { parseBreak, serializeBreak } from '../breakParser';
import { ParseErrorWriter } from '../logParser';
...
describe('parseBreak — valid payloads', () => {
  it('parses a complete valid payload', async () => {
    const result = await parseBreak(validPayload);
  ...
  it('accepts a Date object as timestamp', async () => {
    ...
    const result = await parseBreak({ ...validPayload, timestamp: ts });
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  it('accepts a numeric timestamp (epoch ms)', async () => {
    const ts = new Date('2024-06-01T00:00:00.000Z');
    const result = await parseBreak({ ...validPayload, timestamp: ts.getTime() });
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  it('accepts all valid severities', async () => {
    for (const sev of ['info', 'warning', 'error', 'critical'] as const) {
      const result = await parseBreak({ ...validPayload, severity: sev });
      expect(result.success).toBe(true);
    }
  });
  ...
  it('defaults endpoint to null when missing', async () => {
    ...
    const result = await parseBreak(noEndpoint);
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  it('defaults requestPayload to null when missing', async () => {
    ...
    const result = await parseBreak(noReq);
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  it('defaults userSession to null when missing', async () => {
    ...
    const result = await parseBreak(noSession);
    expect(result.success).toBe(true);
    if (!result.success) return;
...
describe('parseBreak — malformed payloads', () => {
  it('returns error for null payload', async () => {
    ...
    const result = await parseBreak(null, writer);
  ...
  it('returns error for non-object payload', async () => {
    const { writer, calls } = makeWriter();
    const result = await parseBreak('not an object', writer);
    expect(result.success).toBe(false);
    expect(calls).toHaveLength(1);
  });
  ...
  it('returns error for array payload', async () => {
    const { writer, calls } = makeWriter();
    const result = await parseBreak([], writer);
    expect(result.success).toBe(false);
    expect(calls).toHaveLength(1);
  });
  ...
  it('returns error when id is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noId, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error when applicationId is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noApp, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error when environment is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noEnv, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error for invalid severity', async () => {
    const { writer, calls } = makeWriter();
    const result = await parseBreak({ ...validPayload, severity: 'debug' }, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error when errorMessage is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noMsg, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error when stackTrace is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noStack, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error when fingerprint is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noFp, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('returns error for invalid timestamp string', async () => {
    const { writer, calls } = makeWriter();
    const result = await parseBreak({ ...validPayload, timestamp: 'not-a-date' }, writer);
    expect(result.success).toBe(false);
    expect(calls).toHaveLength(1);
  });
  ...
  it('returns error when timestamp is missing', async () => {
    ...
    const { writer, calls } = makeWriter();
    const result = await parseBreak(noTs, writer);
    expect(result.success).toBe(false);
    if (result.success) return;
  ...
  it('does not throw on empty object', async () => {
    ...
    await expect(parseBreak({}, writer)).resolves.toMatchObject({ success: false });
  });

  it('writes to errorWriter exactly once per malformed payload', async () => {
    const { writer, calls } = makeWriter();
    await parseBreak({ ...validPayload, id: '' }, writer);
    expect(calls).toHaveLength(1);
  });
});
...
describe('serializeBreak', () => {
  it('serializes a Break to JSON with ISO timestamp', () => {
    ...
    const json = serializeBreak(record);
  ...
  it('produces valid JSON', () => {
    ...
    expect(() => JSON.parse(serializeBreak(record))).not.toThrow();
  });
});
...
describe('parse → serialize → parse round-trip', () => {
  it('produces an equivalent record', async () => {
    const result1 = await parseBreak(validPayload);
    expect(result1.success).toBe(true);
    if (!result1.success) return;

    const json = serializeBreak(result1.record);
    const result2 = await parseBreak(JSON.parse(json));
  ...
  it('round-trips a Break with null optional fields', async () => {
    ...
    const result1 = await parseBreak(minimalPayload);
    expect(result1.success).toBe(true);
    if (!result1.success) return;
