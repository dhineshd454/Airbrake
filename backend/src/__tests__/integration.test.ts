...
import { ErrorAggregator } from '../aggregator/errorAggregator';
...
function makeInMemoryRedis() {
  ...
  const handlers: Record<string, Array<(msg: string) => void>> = {};
  ...
  return {
    ...
    subscribe: jest.fn((channel: string, handler: (msg: string) => void) => {
      if (!handlers[channel]) handlers[channel] = [];
      handlers[channel].push(handler);
    }),
    unsubscribe: jest.fn(),
    getMessages: (channel: string) => messages[channel] ?? [],
  };
}

function makeReplayStore() {
  const stored: Array<{ channel: string; message: string; timestamp: Date }> = [];
  return {
    store: jest.fn(async (channel: string, message: string, timestamp: Date) => {
      stored.push({ channel, message, timestamp });
    }),
    getRecent: jest.fn(async (channel: string, since: Date) =>
      stored
...
describe('Flow: Log ingestion → stream delivery', () => {
  it('ingested log is published to Redis and delivered to WebSocket clients', async () => {
    ...
    const indexedLogs: LogRecord[] = [];
    const parseErrors: unknown[] = [];

    const pipeline = createLogPipeline(
      { save: jest.fn(async (r) => { savedLogs.push(r); }) },
      { indexLogRecord: jest.fn(async (r) => { indexedLogs.push(r); }) },
      redis,
      { save: jest.fn(async (raw, msg) => { parseErrors.push({ raw, msg }); }) },
    );

    const wsServer = new WebSocketServer(redis, replayStore);
    const receivedMessages: string[] = [];
    const mockClient = { send: jest.fn((msg: string) => { receivedMessages.push(msg); }), isAlive: true };
...
describe('Flow: Break ingestion → aggregation → alert dispatch', () => {
  it('new break triggers alert when threshold is met', async () => {
    const dispatched: Array<{ channel: unknown; event: unknown }> = [];
    const failed: string[] = [];

    const alertEngine = new AlertEngine(
      { countBreaksInWindow: jest.fn(async () => 5) }, // count = 5 >= threshold 3
      { send: jest.fn(async (ch, ev) => { dispatched.push({ channel: ch, event: ev }); }) },
      { markFailed: jest.fn(async (ruleId) => { failed.push(ruleId); }) },
      async () => {}, // no-op delay
    );
    ...
    const rule = {
      ...
      channels: [{ type: 'webhook' as const, url: 'https://example.com/hook' }],
      createdBy: 'user-1',
      enabled: true,
    };
  ...
  it('new error break triggers alert when triggerOnNewError is set', async () => {
    ...
    const alertEngine = new AlertEngine(
      { countBreaksInWindow: jest.fn(async () => 0) }, // below threshold
      { send: jest.fn(async (_ch, ev) => { dispatched.push(ev); }) },
      { markFailed: jest.fn() },
      async () => {},
    );
    ...
    const rule = {
      ...
      channels: [{ type: 'slack' as const, webhookUrl: 'https://hooks.slack.com/test' }],
      createdBy: 'user-1',
      enabled: true,
    };
  ...
  it('disabled rule is not evaluated', async () => {
    ...
    const alertEngine = new AlertEngine(
      { countBreaksInWindow: jest.fn(async () => 999) },
      { send: jest.fn(async (_ch, ev) => { dispatched.push(ev); }) },
      { markFailed: jest.fn() },
      async () => {},
    );
    ...
    const disabledRule = {
      ...
      windowSeconds: 60,
      triggerOnNewError: false,
      channels: [{ type: 'email' as const, address: 'ops@example.com' }],
      createdBy: 'user-1',
      enabled: false,
    };
