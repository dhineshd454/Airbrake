import { AirbrakeClient, HttpClient, RedisPublisher, encryptApiKey, decryptApiKey } from '../airbrakeClient';
...
function makeHttpClient(impl: (url: string, headers: Record<string, string>) => Promise<unknown>): HttpClient {
  return { get: jest.fn(impl) };
}

function makeRedisPublisher(): { publisher: RedisPublisher; calls: Array<{ channel: string; message: string }> } {
...
function makeClient(
  ...
  return new AirbrakeClient(
    { apiKey: 'my-secret-api-key', projectId: 'proj-42', pollIntervalMs },
    httpClient,
    redisPublisher,
  );
}
...
describe('AirbrakeClient', () => {
  ...
  describe('retry logic', () => {
    ...
    it('publishes a break when HTTP client fails twice then succeeds on 3rd attempt', async () => {
      ...
      const pollPromise = (client as unknown as { poll(): Promise<void> }).poll();
  ...
  describe('API key encryption', () => {
    ...
    it('stores the API key encrypted in memory (not as plaintext)', () => {
      ...
      const encryptedField = (client as unknown as { encryptedApiKey: { ciphertext: string; iv: string; tag: string } }).encryptedApiKey;
    ...
    it('encryptApiKey / decryptApiKey round-trips correctly', () => {
      ...
      const encrypted = encryptApiKey(plaintext, passphrase);
      ...
      const decrypted = decryptApiKey(encrypted, passphrase);
  ...
  describe('publish to Redis', () => {
    it('calls redisPublisher.publish with channel="breaks" and valid JSON for a valid break', async () => {
      ...
      // Must be valid JSON
      const parsed = JSON.parse(calls[0].message) as Record<string, unknown>;
  ...
  describe('onBreak handler', () => {
    ...
    it('calls all registered handlers when multiple handlers are registered', async () => {
      ...
      const received1: Break[] = [];
      const received2: Break[] = [];
