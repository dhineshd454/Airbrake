...
import type { AlertRule, AlertEvent, Break, NotificationChannel } from '@portal/shared';
...
function makeDeps(breakCount = 0) {
  const noopDelay: DelayFn = jest.fn().mockResolvedValue(undefined);

  const breakCountRepo: BreakCountRepository = {
    countBreaksInWindow: jest.fn().mockResolvedValue(breakCount),
  };

  const dispatchedCalls: Array<{ channel: NotificationChannel; event: AlertEvent }> = [];
  ...
  const failedCalls: Array<{ ruleId: string; event: AlertEvent }> = [];
  const alertNotificationRepo: AlertNotificationRepository = {
...
describe('AlertEngine.evaluate()', () => {
  it('dispatches when break count >= threshold', async () => {
    ...
    const engine = new AlertEngine(breakCountRepo, dispatcher, alertNotificationRepo, noopDelay);
  ...
  it('evaluates multiple rules independently', async () => {
    ...
    expect(dispatcher.send).toHaveBeenCalledTimes(1);
    const callArg = (dispatcher.send as jest.Mock).mock.calls[0][1] as AlertEvent;
  ...
  it('includes breakCount in the dispatched event', async () => {
    ...
    const event = (dispatcher.send as jest.Mock).mock.calls[0][1] as AlertEvent;
...
describe('AlertEngine.dispatch()', () => {
  ...
  it('retries on failure up to 3 attempts', async () => {
    const noopDelay: DelayFn = jest.fn().mockResolvedValue(undefined);
    const breakCountRepo: BreakCountRepository = { countBreaksInWindow: jest.fn() };
    const alertNotificationRepo: AlertNotificationRepository = { markFailed: jest.fn().mockResolvedValue(undefined) };
  ...
  it('applies exponential backoff delays between retries', async () => {
    const delayFn: DelayFn = jest.fn().mockResolvedValue(undefined);
    const breakCountRepo: BreakCountRepository = { countBreaksInWindow: jest.fn() };
    const alertNotificationRepo: AlertNotificationRepository = {
      markFailed: jest.fn().mockResolvedValue(undefined),
    };
    ...
    const engine = new AlertEngine(breakCountRepo, dispatcher, alertNotificationRepo, delayFn);
    const rule = makeRule();
