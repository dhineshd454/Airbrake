...
function makeClient(): WebSocketClient & { messages: string[] } {
...
function makePubSub(): PubSubSubscriber & {
  handlers: Map<string, (msg: string) => void>;
...
function makeReplayStore(): EventReplayStore & {
  stored: Array<{ channel: string; message: string; timestamp: Date }>;
} {
...
describe('WebSocketServer', () => {
  ...
  let replayStore: ReturnType<typeof makeReplayStore>;
  let server: WebSocketServer;

  beforeEach(() => {
    pubSub = makePubSub();
    replayStore = makeReplayStore();
    server = new WebSocketServer(pubSub, replayStore);
  });
  ...
  describe('removeClient', () => {
    it('sends a Disconnected state message to the removed client', () => {
      ...
      expect(client.messages).toHaveLength(1);
      expect(client.messages[0]).toBe(DISCONNECT_MESSAGE);
      const parsed = JSON.parse(client.messages[0]);
