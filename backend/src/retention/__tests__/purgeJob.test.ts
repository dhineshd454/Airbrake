import { PurgeJob } from '../purgeJob';
import type { PurgeRepository, RetentionPolicyReader } from '../purgeJob';
import type { RetentionPolicy } from '@portal/shared';

// ─── Helpers ──────────────────────────────────────────────────────────────────

function makePurgeRepo(logsDeleted = 0, breaksDeleted = 0): PurgeRepository {
  return {
...
function makePolicyReader(policies: RetentionPolicy[]): RetentionPolicyReader {
  return {
...
describe('PurgeJob.run()', () => {
  it('calls deleteLogsBefore with correct cutoff for a 30-day policy', async () => {
    ...
    const job = new PurgeJob(repo, makePolicyReader([{ applicationId: 'app-1', retentionDays: 30 }]));
  ...
  it('returns correct total counts from a single policy', async () => {
    ...
    const job = new PurgeJob(repo, makePolicyReader([{ applicationId: 'app-1', retentionDays: 60 }]));
  ...
  it('handles multiple retention policies and sums totals', async () => {
    ...
    const job = new PurgeJob(repo, makePolicyReader(policies));
    const result = await job.run(NOW);
  ...
  it('does nothing and returns zeros when there are no policies', async () => {
    ...
    const job = new PurgeJob(repo, makePolicyReader([]));
