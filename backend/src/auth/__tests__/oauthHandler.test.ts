...
function makeSessionStore(initial: Record<string, SessionData> = {}): SessionStore {
  const store = new Map<string, SessionData>(Object.entries(initial));
  return {
...
function makeUserRepo(users: User[] = []): UserRepository {
  const db = new Map<string, User>(users.map(u => [`${u.oauthProvider}:${u.oauthSubject}`, u]));
  return {
...
function makeOAuthProvider(userInfo = { subject: 'sub-1', email: 'dev@example.com', provider: 'google' }): OAuthProvider {
  return {
...
describe('createSession', () => {
  it('returns a non-empty token string', async () => {
    ...
    const token = await createSession('user-1', 'developer', store);
  ...
  it('stores session data with correct userId and role', async () => {
    const store = makeSessionStore();
    const token = await createSession('user-42', 'admin', store);
  ...
  it('sets expiresAt ~24 hours in the future', async () => {
    ...
    const store = makeSessionStore();
    const token = await createSession('user-1', 'viewer', store);
    const data = await store.get(token);
  ...
  it('generates unique tokens for each call', async () => {
    const store = makeSessionStore();
    const t1 = await createSession('user-1', 'viewer', store);
    const t2 = await createSession('user-1', 'viewer', store);
...
describe('getSession', () => {
  it('returns session data for a valid token', async () => {
    const store = makeSessionStore();
    const token = await createSession('user-1', 'developer', store);
    const data = await getSession(token, store);
  ...
  it('returns null for an unknown token', async () => {
    const store = makeSessionStore();
    const data = await getSession('nonexistent-token', store);
  ...
  it('returns null and deletes an expired session', async () => {
    ...
    const data = await getSession('expired-token', store);
    expect(data).toBeNull();
...
describe('handleOAuthCallback', () => {
  it('creates a session and returns a token for a new user', async () => {
    ...
    const token = await handleOAuthCallback('auth-code', 'state', store, userRepo, provider);
    expect(typeof token).toBe('string');
    expect(token.length).toBeGreaterThan(0);

    const session = await getSession(token, store);
...
describe('enforceHttps', () => {
  const makeRes = () => {
    const res = {
      redirected: '',
      redirect(url: string) { this.redirected = url; },
      status(_code: number) { return { send(_body: string) {} }; },
    };
    return res;
  };
