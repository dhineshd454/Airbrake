...
function countBreaksInWindow(breaks: Break[], windowHours: number, now: Date): number {
...
function getTopServices(
  breaks: Break[],
  limit: number,
): Array<{ applicationId: string; count: number }> {
...
function groupBySeverity(breaks: Break[]): Record<string, number> {
...
const arbitraryBreak = (
  timestampArb?: fc.Arbitrary<Date>,
): fc.Arbitrary<Break> =>
  fc.record({
    id: fc.uuid(),
...
describe('Property 8: Top Services Ranking', () => {
  ...
  it('getTopServices returns services in descending order of break count', () => {
    fc.assert(
      fc.property(
        fc.array(arbitraryBreak(), { maxLength: 60 }),
...
describe('Property 9: Bucketing Preserves Total Count', () => {
  ...
  const arbitraryBreakInWindow = arbitraryBreak(
    fc.date({ min: WINDOW_FROM, max: WINDOW_TO }),
  );

  it('sum of hourly bucket counts equals total break count', () => {
    fc.assert(
      fc.property(
        fc.array(arbitraryBreakInWindow, { maxLength: 60 }),
        (breaks) => {
